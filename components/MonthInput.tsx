...
import { Plus, Trash2, ChevronLeft, ChevronRight, Pencil, Check, X, Repeat } from 'lucide-react';
...
export const MonthInput: React.FC<MonthInputProps> = ({ type, yearData, user, onUpdate }) => {
  ...
  const applyRecurring = (category: string, value: number, isCustom: boolean) => {
    if (!window.confirm(`Voulez-vous appliquer ce montant (${value}) à tous les mois suivants de l'année pour "${category}" ?`)) return;

    const newYearData = JSON.parse(JSON.stringify(yearData));
    // Explicitly define order to ensure safety
    ...
    for (let i = currentIndex + 1; i < allMonths.length; i++) {
      const nextMonthKey = allMonths[i];
      const targetMonth = newYearData.months[nextMonthKey];
      
      if (targetMonth) {
        const targetSection = targetMonth[type];
        
        // Ensure custom category exists in future months if it's custom
        if (isCustom) {
          targetSection.custom[category] = Number(value);
        } else {
          targetSection.base[category] = Number(value);
        }
        updateCount++;
      }
    }
    ...
    if (updateCount > 0) {
      onUpdate(newYearData);
      alert(`${updateCount} mois ont été mis à jour avec succès.`);
    } else {
      alert("Aucun mois suivant à mettre à jour (vous êtes peut-être déjà en décembre ?).");
    }
  };
  ...
  const annualTotal = Number(calculateYearTotal(type, yearData));
  const monthlyTotal = Number(calculateMonthTotal(type, currentMonthData));
  ...
  const renderCategoryRow = (cat: string, val: number, isCustom: boolean) => {
    ...
    return (
      <div key={cat} className="flex items-center justify-between group h-10">
        ...
        {/* Amount Input */}
        <div className="relative w-48 flex items-center space-x-2">
          ...
          {/* Recurring Button */}
          {!isEditing && currentIndex < 11 && (
             <button 
               onClick={() => applyRecurring(cat, val, isCustom)}
               className="p-2 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors"
               title="Appliquer ce montant aux mois suivants"
             >
               <Repeat size={16} />
             </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-800 space-y-2">
            <p>ℹ️ Les modifications sont enregistrées automatiquement.</p>
            <p>🔄 Utilisez le bouton <Repeat size={12} className="inline"/> pour copier un montant sur tous les mois restants de l'année.</p>
          </div>
        </div>
      </div>
    </div>
