...
const StatCard: React.FC<{ 
  ...
  <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center space-x-4 overflow-hidden">
    <div className={`p-3 rounded-full ${color} text-white shrink-0`}>
      {icon}
    </div>
    <div className="min-w-0 flex-1">
      <p className="text-sm text-slate-500 font-medium uppercase tracking-wide truncate">{title}</p>
      <p 
        className="text-lg md:text-xl font-bold text-slate-800 truncate" 
        title={isHidden ? '' : formatMoney(amount, currency)}
      >
